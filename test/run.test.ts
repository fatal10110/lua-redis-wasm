...
test("eval + redis.call + binary safety + error paths", async () => {
  ...
  const echoResult = engine.eval("return redis.call('ECHO', string.char(97,0,98))");
...
test("limits: maxArgBytes", async () => {
  ...
  const limitResult = limitedEngine.evalWithArgs("return 1", [Buffer.from("a")], [Buffer.from("b")]);
  assert.ok(limitResult && typeof limitResult === "object" && "err" in limitResult);
});
